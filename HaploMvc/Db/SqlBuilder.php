...
use HaploMvc\Exception\InvalidComparisonOperatorException;
use HaploMvc\Exception\InvalidSortOrderException;
use HaploMvc\Exception\InvalidJoinTypeException;
use HaploMvc\Exception\InvalidParameterException;
...
class SqlBuilder
{
     ...
     *
     * @throws \HaploMvc\Exception\InvalidComparisonOperatorException
     * @throws \HaploMvc\Exception\InvalidParameterException
     * @return SqlBuilder $this
     */
    protected function _where()
    {
        ...
        if (count($args) === 0) {
            throw new InvalidParameterException('Invalid parameters specified.');
        }
        ...
        if (!in_array($comparison, $this->comparisonOperators)) {
            throw new InvalidComparisonOperatorException('Invalid comparison operator specified.');
        }
    ...
    {
        $args = array_merge(['AND'], func_get_args());
        return call_user_func_array([$this, '_where'], $args);
    }
    ...
    {
        $args = array_merge(['OR'], func_get_args());
        return call_user_func_array([$this, '_where'], $args);
    }
     ...
     * @param string $order
     * @return SqlBuilder $this
     * @throws \HaploMvc\Exception\InvalidSortOrderException
     */
    public function orderBy($field, $order = 'ASC')
    {
        if (!in_array($order, ['ASC', 'DESC'])) {
            throw new InvalidSortOrderException('Invalid sort order specified.');
        }
    ...
    protected function _join($table, $type = 'JOIN')
    {
        ...
        if (!in_array($type, $this->joinTypes)) {
            throw new InvalidJoinTypeException('Invalid join type specified.');
        }
